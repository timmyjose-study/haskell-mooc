...
data Position = MkPosition Int Int deriving Show
...
study (NthYear n) | n < 7 = NthYear (n + 1)
                  | otherwise = Graduated
...
rgb(Mix c1 c2) = mixRgb (rgb c1) (rgb c2)
    where
      mixRgb [r1, g1, b1] [r2, g2, b2] = [(r1 + r2) / 2, (g1 + g2) / 2, (b1 + b2) / 2] 
rgb (Invert c) = invertColor (rgb c)
  where
      invertColor [r, g, b] = [1 - r, 1 - g, 1 - b]
