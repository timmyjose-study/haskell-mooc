...
firstHalf s = take len s
  where
    len = if even (length s)
             then length s `div` 2
             else length s `div` 2 + 1
...
capitalize = unwords . map (\(w : ws) -> toUpper w : ws) . words
...
powers :: Int -> Int -> [Int]
powers k max = map (k^) $ takeWhile (\e -> k^e <= max) [0..]
...
joinToLength n s = [c | l <- s, r <- s, let c = l <> r, length c == n]
...
multiCompose = foldl (\acc f -> acc . f) id 
...
--multiApp :: ([a] -> b) -> [a -> a] -> a -> [a]
multiApp f gs x = f $ reverseMap gs x
  where
    reverseMap [] _ = []
...
interpreter cmds = interpreterHelper cmds 0 0
  where
    ...
    interpreterHelper ("up" : cs) x y = interpreterHelper cs x (y + 1)
    interpreterHelper ("down" : cs) x y = interpreterHelper cs x (y - 1)
    interpreterHelper ("left" : cs) x y = interpreterHelper cs (x - 1) y
    interpreterHelper ("right" : cs) x y = interpreterHelper cs (x + 1) y
    interpreterHelper ("printX" : cs) x y = show x : interpreterHelper cs x y
    interpreterHelper ("printY" : cs) x y = show y : interpreterHelper cs x y
