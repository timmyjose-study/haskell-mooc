...
instance Price a => Price [a] where
  price [] = 0
  price (item : items) = price item + price items
...
instance Eq RationalNumber where
  RationalNumber w x == RationalNumber y z = w * z == x * y
...
instance Num RationalNumber where
  RationalNumber w x + RationalNumber y z =  simplify $ RationalNumber (w * z + x * y) (x * z)
  RationalNumber w x * RationalNumber y z = simplify $ RationalNumber (w * y) (x * z)
  abs (RationalNumber w x) = RationalNumber (abs w) (abs x)
  signum(RationalNumber w x) = RationalNumber (signum w) (signum x)
  fromInteger n = RationalNumber n 1
  negate (RationalNumber w x) = RationalNumber ((0 - w)) (x)
...
class Cycle a where
  ...
  stepMany n x = last . take (n + 1) $ iterate step x
