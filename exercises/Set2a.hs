...
takeFinal :: Int -> [a] -> [a]
takeFinal n xs | n >= length xs = xs
               | otherwise = drop nprime xs
             where
                    nprime = length xs - n
...
updateAt :: Int -> a -> [a] -> [a]
updateAt i x xs = updateAtHelper i x xs 0
  where
    updateAtHelper i x xs idx | idx == i = x : tail xs
                              | otherwise = head xs : updateAtHelper i x (tail xs) (idx + 1)
...
substring i j s = substringHelper i j s 0
  where
    substringHelper i j s start | start == i = take (j - i) s
                                | otherwise = substringHelper i j (tail s) (start + 1)
...
palindromify s | isPalindrome s = s
               | otherwise = palindromify (substring 1 len s)
              where
                len = length s - 1
...
greet first last = case last of
                     Nothing -> "Hello, " <> first <> "!"
                     Just surname -> "Hello, " <> first <> " " <> surname <> "!"
